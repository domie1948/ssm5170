      ...
      <form class="layui-form" style="margin: 10px 0;">
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="shijianduan" id="shijianduan" placeholder="时间段" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('yuyueshijianduan','新增')" @click="jump('../yuyueshijianduan/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../yuyueshijianduan/detail.jsp?id='+item.id">
              <figure>
              </figure>
              <div class="div1">
                <b></b>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#shijianduan').val()) {
          param['shijianduan'] = jquery('#shijianduan').val() ? '%' + jquery('#shijianduan').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('yuyueshijianduan/list', 'get', param, function(res) {
